    ...
    public boolean onCommand(CommandSender commandSender, Command command, String string, String[] strings) {
        if(string.equalsIgnoreCase("map")) {
            ...
            if(strings[0].equalsIgnoreCase("create")) {
                if(strings.length != 2) {
                    player.sendMessage("Usage: /map create <map_name>");
                } else {
                    player.sendMessage("Creating map '" + strings[1] + "'...");
                    if(mm.createNewMap(strings[1], player.getName(), player.getWorld().getName())) {
                        player.sendMessage("Created blank map " + strings[1] + "'!");
                    } else {
                        player.sendMessage("Map '" + strings[1] + "' already exists.");
                        player.sendMessage("Try /map delete <map_name> to clear the map!");
                    }
                }
                return true;
            } else if(strings[0].equalsIgnoreCase("delete")) {
                if(strings.length != 2) {
                    player.sendMessage("Usage: /map delete <map_name>");
                } else {
                    player.sendMessage("Deleting map '" + strings[1] + "'...");
                    if(mm.deleteMap(strings[1])) {
                        player.sendMessage("Deleted map '" + strings[1] + "'!");
                    } else {
                        player.sendMessage("Map '" + strings[1] + "' doesn't exist.");
                        player.sendMessage("Try /map create <map_name> to create the map!");
                    }
                }
                return true;
            } else if(strings[0].equalsIgnoreCase("setspawn")) {
                if(strings.length != 3) {
                    player.sendMessage("Usage: /map setspawn <map_name> <spawnpoint_number>");
                } else if(MapManager.getManager().getMap(strings[1]) == null) {
                    player.sendMessage("Could not find map named '" + strings[1] + "'");
                    player.sendMessage("Usage: /map setspawn <map_name> <spawnpoint_number>");
                } else if(Integer.parseInt(strings[2]) <= 0 || Integer.parseInt(strings[2]) > 10) {
                    player.sendMessage("Spawnpoint number must be between 1 and 10!");
                    player.sendMessage("Usage: /map setspawn <map_name> <spawnpoint _number>");
                } else {
                    GameMap m = mm.getMap(strings[1]);
                    m.setSpawnLocation(Integer.parseInt(strings[2]), player.getLocation());
                    player.sendMessage("Spawn point #" + strings[2] + " set!");
                }
                return true;
            } else if(strings[0].equalsIgnoreCase("setname")) {
                if(strings.length != 3) {
                    player.sendMessage("Usage: /map setname <old_name> <new_name>");
                } else {
                    if(mm.getMap(strings[1]) != null) {
                        mm.getMap(strings[1]).setName(strings[2]);
                        player.sendMessage("Renamed map '" + strings[1] + "' to '" + strings[2] + "'!");
                    } else {
                        player.sendMessage("Could not find map '" + strings[1] + "'!");
                    }
                }
                return true;
            } else if(strings[0].equalsIgnoreCase("setcreator")) {
                if(strings.length != 3) {
                    player.sendMessage("Usage: /map setname <map_name> <creator_name>");
                } else {
                    if(mm.getMap(strings[1]) != null) {
                        mm.getMap(strings[1]).setCreator(strings[2]);
                        player.sendMessage("Changed creator of map '" + strings[1] + "' to '" + strings[2] + "'!");
                    } else {
                        player.sendMessage("Could not find map '" + strings[1] + "'!");
                    }
                }
                ...
                if(strings.length != 2) {
                    player.sendMessage("Usage: /map verify <map_name>");
                } else {
                    if(mm.getMap(strings[1]) != null && mm.getMap(strings[1]).verifySpawnLocations()) {
                        player.sendMessage("Map '" + strings[1] + "' is valid and playable!");
                    } else if(mm.getMap(strings[1]) == null){
                        player.sendMessage("Could not find map '" + strings[1] + "'!");
                    } else {
                        player.sendMessage("Map '" + strings[1] + "' is not currently valid!");
                    }
                }
                return true;
            }
